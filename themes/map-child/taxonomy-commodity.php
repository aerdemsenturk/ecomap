...
<div class="wrapper" id="modal-ready">
	...
	<div class="<?php echo esc_html( $container ); ?>" id="content" tabindex="-1">
		...
		<header class="page-header">
			...
			$commodity_image = get_field('commodity_image', 'commodity_' . $term_object->term_id . '' );

			// commodity image
			echo '<div class="row">';
			echo '<div class="col-6"><img src="' . $commodity_image['sizes']['thumbnail'] . '" class="rounded-circle" alt="' . $commodity_image['alt'] .'"></div>';
			echo '</div>';
						...
						if ( ! empty( $commodity_terms ) ) {
							if ( ! is_wp_error( $commodity_terms ) ) {
									foreach( $commodity_terms as $term ) {
										echo '<a target="_parent" href="' . get_term_link( $term->slug, 'commodity' ) . '">●&nbsp;' . esc_html( $term->name ) . '&nbsp;</a>'; 
									}
							}
						}
						?>
